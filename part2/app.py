...
def find_spans(text: str, pattern: str):
    ...
    #  - start is inclusive and end is exclusive, so text[start:end] == pattern at that match.
    #  - count overlapping matches (advance the scan by 1 after a match).
    #  - assume inputs are already lowercased; indices must be relative to `text` as given.
    #  - if pattern is empty or longer than text, return an empty list.
